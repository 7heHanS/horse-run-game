"""
Fine-tuning pipeline V5.
Loads existing model_v4.pt and fine-tunes on the highly-curated MCTS overnight dataset.
Places massive weight on 'is_critical' flag to instantiate the killer instinct.
"""
import torch
...
class HorseGameDatasetV5(Dataset):
    def __init__(self, csv_file, winner_only=True):
        ...
        print(f"Pre-computing segregated V5 masks for {len(self.data)} samples...", flush=True)
        self.from_masks = []
        self.to_masks = []
    ...
    def get_sample_weights(self, critical_weight=30.0):
        # Apply intense weight to critical moves
...
def train_v5(csv_file="dataset_mcts_overnight.csv", base_model="model_v4.pt", epochs=30, batch_size=128, model_save_path="model_v5.pt"):
    if not os.path.exists(csv_file):
        print(f"Dataset {csv_file} not found.")
        return

    full_dataset = HorseGameDatasetV5(csv_file, winner_only=True)
    ...
    sample_weights = full_dataset.get_sample_weights(critical_weight=50.0) # Massive weight for killer instinct
    ...
    if os.path.exists(base_model):
        ...
        print(f"Base model {base_model} not found. Training from scratch.")

    total_params = sum(p.numel() for p in model.parameters())
    print(f"V5 Fine-tuning params: {total_params:,}")
    ...
    print("\nStarting V5 Fine-Tuning...", flush=True)
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"  --> Best Model V5 saved", flush=True)
            patience_counter = 0
        else:
            patience_counter += 1
